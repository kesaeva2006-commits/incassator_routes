...
from atm import Atm
# Импортируем функцию генерации банкоматов
...
# Тест 4: проверяет, что функция корректно работает с пустым списком
...
# Тест 5: проверяет, что функция работает когда банкоматов ровно столько же сколько кластеров
def test_cluster_few_atms():
    # Генерируем ровно 5 банкоматов — минимум для 5 кластеров
    atms = generate_atms(5)
    clusters = cluster_atms(atms, n_clusters=5)
    # Всего 5 кластеров
    assert len(clusters) == 5
    # Суммарное количество банкоматов в кластерах должно быть 5
